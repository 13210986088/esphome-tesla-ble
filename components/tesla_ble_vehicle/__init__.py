...
NUMBERS = [
    {"id": "charging_amps", "name": "Charging Amps", "class": TeslaChargingAmpsNumber, "setter": "set_charging_amps_number", "icon": "mdi:current-ac", "unit": "A", "min": 0, "max": "config", "step": 1},
    {"id": "charging_limit", "name": "Charging Limit", "class": TeslaChargingLimitNumber, "setter": "set_charging_limit_number", "icon": "mdi:battery-charging-100", "unit": "%", "min": 50, "max": 100, "step": 1},
]
...
async def to_code(config):
    ...
    for definition in BINARY_SENSORS: await create_binary_sensor(var, definition)
    for definition in SENSORS: await create_sensor(var, definition)
    for definition in TEXT_SENSORS: await create_text_sensor(var, definition)
